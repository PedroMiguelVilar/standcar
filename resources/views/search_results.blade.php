...
@section('content')
    <link rel="stylesheet" href="{{ asset('css/show_results.css') }}">
    ...
    <div class="car-showcase">
      <div class="car-grid">
          ...
          <div class="car-card">
              <a href="{{ route('car', ['id' => $result->id]) }}">
                  <div id="car-carousel-{{ $result->id }}" class="carousel slide" data-bs-slide="prev">
                      <div class="carousel-inner">
                          @php $firstPhoto = true; @endphp
                          @foreach ($car_photos as $car_photo)
                              @if ($car_photo->car_id == $result->id)
                      ...
                      <button class="carousel-control-next" type="button" data-bs-target="#car-carousel-{{ $result->id }}" data-bs-slide="next"></button>
                  </div>
                  <div class="car-details">
                      <h3 class="car-title">{{ $result->brand }}</h3>
                      <div class="car-info">
                          <div class="car-model">{{ $result->model }}</div>
                          <div class="car-extra">
                              <span class="car-mileage">{{ number_format($result->mileage, 0, ',', '.') }}Km</span>
